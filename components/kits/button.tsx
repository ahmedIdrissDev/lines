...
const Button = ({icon , label ,path}:linksProps) => {
  ...
  return (
   <button onClick={handleNavigation} className={twMerge('w-full relative duration-300 h-11 px-1.5 flex rounded-md    cursor-pointer items-center justify-start  gap-2' , isActive ? 'bg-tgcc-200  ':" opacity-60 ")}>
        {icon}
