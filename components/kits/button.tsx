...
const Button = ({icon , label ,path}:linksProps) => {
  ...
  return (
   <button onClick={handleNavigation} className={twMerge('w-full relative duration-300 h-11 px-2 flex rounded-md    cursor-pointer items-center justify-start  gap-2' , isActive ? 'opacity-100 bg-neutral-100  ':" opacity-60 ")}>
        {icon}
