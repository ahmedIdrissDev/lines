...
the brand blue (`{colors.primary}` — `#0c3aff`) feels like a stamp rather
than a notification.
...
- Hot blue (`{colors.primary}` — `#0c3aff`) is reserved for the primary CTA, the hero band, and inline link colour. Never decorative.
...
- Section rhythm: cream → blue hero → cream → dark code-story band → cream → black footer.

## Colors

### Brand & Accent
- **Replicate Blue** (`{colors.primary}` — `#0c3aff`): the brand accent. Reserved for the primary CTA, hero band background, and inline link colour. Treat as a stamp — one blue element per viewport at most.
- **Blue Pressed** (`{colors.primary-deep}` — `#012cff`): the active/pressed state of `{colors.primary}` — used on `{component.button-primary-pressed}`.
- **Hero Glow** (`{colors.hero-glow}` — `#568fff`): the lighter blue that appears in the radial atmospheric mesh behind the hero copy.
- **Hero Pink** (`{colors.hero-pink}` — `#b3d5ff`): a light blue wash that softens the bottom edge of the hero band before it transitions to cream.
...
- **Link** (`{colors.link}` — `#0c3aff`): inline link colour — same as primary blue, intentionally pulling links into the brand accent.
- **Focus Ring** (`{colors.ring-focus}` — `rgba(12,58,255,0.5)`): the default focus ring on interactive elements.
